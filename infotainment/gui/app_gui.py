...
class AppGUI:
    def __init__(self, root):
        ...
        # # === Container Frame for Whole Left & Right Side ===
        # main_pane = tk.Frame(root)
        # main_pane.pack(fill="both", expand=True)

        # left_pane = tk.Frame(main_pane, width=350)  # Set fixed width
        # left_pane.pack(side="left", fill="y", padx=5, pady=5)
        # left_pane.pack_propagate(False)  # 🔑 Prevent resizing

        # script_box = tk.LabelFrame(left_pane, text="Uploaded Scripts", width=350)
        # script_box.pack(fill="both", expand=True, padx=5, pady=5)
        # script_box.pack_propagate(False)

        # canvas = tk.Canvas(script_box, bg="white", highlightthickness=0, width=330)
        # canvas.pack(side="left", fill="both", expand=True)




        # scrollbar = ttk.Scrollbar(script_box, orient="vertical", command=canvas.yview)
        # scrollbar.pack(side="right", fill="y")

        # self.checkbox_container = tk.Frame(canvas, bg="white")
        ...
        # self.canvas = canvas  # So upload_test_scripts() can use it
        # --- Main Split Layout ---
        main_pane = tk.Frame(root)
        main_pane.pack(fill="both", expand=True)

# --- LEFT PANE: Uploaded Scripts ---
        left_pane = tk.Frame(main_pane, width=600, bg="white")
        left_pane.pack(side="left", fill="y", padx=5, pady=5)
        left_pane.pack_propagate(False)

        script_box = tk.LabelFrame(left_pane, text="Uploaded Scripts", bg="white")
        script_box.pack(fill="both", expand=True, padx=5, pady=5)

        canvas = tk.Canvas(script_box, bg="white", highlightthickness=0)
        canvas.pack(side="left", fill="both", expand=True)
...
# --- RIGHT PANE: Output Box ---
        right_pane = tk.Frame(main_pane)
        right_pane.pack(side="right", fill="both", expand=True, padx=5, pady=5)

        output_box = tk.LabelFrame(right_pane, text="Output")
        output_box.pack(fill="both", expand=True)

        self.output_text = tk.Text(output_box, wrap="word", bg="black", fg="white")
        ...
        self.left_pane, self.uploaded_frame = create_output_pane(
        root,
        self.clear_output,
        None
    )
